...
from payments.stripe_api.api import stripe_api as stripe
from payments.stripe_models.customer import StripeCustomers, StripeCustomer
from newsltr.settings import (
...
def _get_or_create_django_user_if_configured(customer: StripeCustomer):
    ...
    django_user_query_filters = {DJANGO_USER_EMAIL_FIELD: customer.email}
    django_user = get_user_model().objects.filter(**django_user_query_filters).first()

    if django_user:
        return django_user, False

    if not USER_CREATE_DEFAULTS_ATTRIBUTE_MAP:
    ...
    defaults = {
        k: getattr(customer, v) for k, v in USER_CREATE_DEFAULTS_ATTRIBUTE_MAP.items()
    }
    defaults[DJANGO_USER_EMAIL_FIELD] = customer.email
    ...
    try:
        ...
        if not django_user:
            ...
            defaults = {
                ...
                for k, v in USER_CREATE_DEFAULTS_ATTRIBUTE_MAP.items()
            }
            defaults[DJANGO_USER_EMAIL_FIELD] = customer.email
            django_user = get_user_model().objects.create(**defaults)
...
def stripe_api_update_customers(limit=100, starting_after=None, test_data=None):
    ...
    for customer in stripe_customers:
        ...
        if customer.email is not None:
            query_filters = {DJANGO_USER_EMAIL_FIELD: customer.email}
            if USER_CREATE_DEFAULTS_ATTRIBUTE_MAP:
                defaults = {
                    k: getattr(customer, v)
                    for k, v in USER_CREATE_DEFAULTS_ATTRIBUTE_MAP.items()
                }
            ...
            if user:
                ...
                print(
                    f"Could not find Stripe Customer id '{customer.id}' in user model '{get_user_model()}' with '{DJANGO_USER_EMAIL_FIELD}' of '{customer.email}', USER_CREATE_DEFAULTS_ATTRIBUTE_MAP is not set so skipping Customer."
                )
