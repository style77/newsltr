...
def _make_stripe_checkout_params(
    ...
    cancel_url = reduce(urljoin, (FRONT_END_BASE_URL, CHECKOUT_CANCEL_URL_PATH))
...
def _make_trial_end_timestamp(trial_end: Union[str, None, datetime] = "auto"):
    ...
    if trial_end == "auto":
        if NEW_USER_FREE_TRIAL_DAYS is None:
            return
        else:
            trial_end = timezone.now() + timezone.timedelta(
                days=NEW_USER_FREE_TRIAL_DAYS + 1
            )
